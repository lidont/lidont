...
@pytest.fixture(scope="session")
def lidont(project, accounts):
    lidont = project.lidont.deploy(accounts[0].address, sender=accounts[0])
    return lidont
...
def mint_lidont(lidont, project, accounts):
    lidont.mint(800 * 10 ** 18, accounts[0].address, sender=accounts[0])
...
def test_rewards_eth_temp_clearing(ETH_pipe, lidont, mint_lidont, setup_allowance, distribute_reward, stake_ETH, accounts):
    assert ETH_pipe.temp() == mint_lidont
    lidont.mint(800 * 10 ** 18, accounts[0].address, sender=accounts[0])
    lidont.transferFrom("0x0000000000000000000000000000000000000000", accounts[0], 800 * 10 ** 18, sender=accounts[0])
    ETH_pipe.receiveReward(lidont.address, accounts[0].address, mint_lidont, sender=accounts[0])
...
def test_rewards_eth_dust_clearing(ETH_pipe, lidont, mint_lidont, setup_allowance, stake_ETH, distribute_reward, accounts):
    assert ETH_pipe.dust() > 0
    dust = ETH_pipe.dust()
    totalBonds = ETH_pipe.totalStake() // 10 ** 9
    perfectReward = totalBonds * 2
    rewardsToClear = perfectReward - dust
    lidont.mint(rewardsToClear, accounts[0].address, sender=accounts[0])
