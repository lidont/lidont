...
customElements.define("select-pipes", class extends HTMLElement {
  ...
  render(){
    ...
    ${pipes.map( (value, index) => { 
      let label
      if(index === 0) label = "ETH"
      if(index === 1) label = "rETH(broken)"
      if(index === 2) label = "rETH(new)"
      return html`
...
customElements.define("list-deposits", class extends HTMLElement {
  ...
  render(requests){
    ...
    <sub>lido final: <value-connected hideSpinner data-path="lastFinalizedRequestId"></value-connected></sub>
    <hr/>
      ${ids.map( (id, index) => { 
        ...
        return html`
          ...
          ${reqId.isFinalized === true && reqId.isClaimed !== true ? '<sub class="flex-right">⚠️ lido done. finalize & claim ⚠️</sub>' : ''}
